...
class EventFilter(django_filters.FilterSet):
    ...
    def custom_finished(self, queryset, name, value):
        if value:
            return queryset.filter(end_date__lt=datetime.datetime.now()).order_by('-registred_at')
        return queryset.filter(end_date__gte=datetime.datetime.now()).order_by('-registred_at')
    ...
    def custom_filters(self, queryset, name, value):
        ...
        qs = queryset.annotate(
            concatenated_fields=Concat('event_type__name',
                                'location__street',
                                'category__name', output_field=CharField()))
